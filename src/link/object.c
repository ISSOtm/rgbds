...
#define tryRead(func, type, errval, var, file, ...)                                       \
	do {                                                                              \
		...
		if (tmpVal == (errval)) {                                                 \
			errx(1, __VA_ARGS__,                                              \
			     feof(tmpFile) ? "Unexpected end of file" : strerror(errno)); \
		}                                                                         \
		var = tmpVal;                                                             \
	} while (0)
 ...
 */
#define tryReadlong(var, file, ...) tryRead(readlong, int64_t, INT64_MAX, var, file, __VA_ARGS__)
 ...
 *            argument is provided, the reason for failure
 */
#define tryFgetc(var, file, ...) tryRead(fgetc, int, EOF, var, file, __VA_ARGS__)
 ...
 *            argument is provided, the reason for failure
 */
#define tryGetc(var, file, ...) tryRead(getc, int, EOF, var, file, __VA_ARGS__)
 ...
 *            argument is provided, the reason for failure
 */
#define tryReadstr(var, file, ...) tryRead(readstr, char *, NULL, var, file, __VA_ARGS__)
                              ...
                              char const *fileName)
{
	...
	tryReadlong(parentID, file, "%s: Cannot read node #%" PRIu32 "'s parent ID: %s", fileName,
	            i);
	...
	tryGetc(fileNodes[i].type, file, "%s: Cannot read node #%" PRIu32 "'s type: %s", fileName,
	        i);
	...
	case NODE_REPT:
		...
		if (!fileNodes[i].parent)
			fatal(NULL, 0,
			      "%s is not a valid object file: root node (#%" PRIu32
			      ") may not be REPT",
			      fileName, i);
	}
}
...
{
	tryReadstr(symbol->name, file, "%s: Cannot read symbol name: %s", fileName);
	tryGetc(symbol->type, file, "%s: Cannot read \"%s\"'s type: %s", fileName, symbol->name);
	...
	if (symbol->type != SYMTYPE_IMPORT) {
		...
		tryReadlong(symbol->sectionID, file, "%s: Cannot read \"%s\"'s section ID: %s",
		            fileName, symbol->name);
		tryReadlong(symbol->offset, file, "%s: Cannot read \"%s\"'s value: %s", fileName,
		            symbol->name);
	} else {
		symbol->sectionID = -1;
	}
                      ...
                      uint32_t i, struct Section *fileSections[], struct FileStackNode fileNodes[])
{
	...
	tryReadlong(patch->pcOffset, file,
	            "%s: Unable to read \"%s\"'s patch #%" PRIu32 "'s PC offset: %s", fileName,
	            sectName, i);
	tryGetc(patch->type, file, "%s: Unable to read \"%s\"'s patch #%" PRIu32 "'s type: %s",
	        fileName, sectName, i);
	tryReadlong(patch->rpnSize, file,
	            "%s: Unable to read \"%s\"'s patch #%" PRIu32 "'s RPN size: %s", fileName,
	            sectName, i);

	patch->rpnExpression = malloc(sizeof(*patch->rpnExpression) * patch->rpnSize);
	if (!patch->rpnExpression)
		err(1, "%s: Failed to alloc \"%s\"'s patch #%" PRIu32 "'s RPN expression", fileName,
		    sectName, i);
	size_t nbElementsRead =
	    fread(patch->rpnExpression, sizeof(*patch->rpnExpression), patch->rpnSize, file);
	...
	if (sect_HasData(section->type)) {
		...
		if (section->size) {
			...
			if (nbElementsRead != section->size)
				errx(1, "%s: Cannot read \"%s\"'s data: %s", fileName,
				     section->name,
				     feof(file) ? "Unexpected end of file" : strerror(errno));
		}
		section->data = data;
		...
		for (uint32_t i = 0; i < section->nbPatches; i++) {
			readPatch(file, &patches[i], fileName, section->name, i, fileSections,
			          fileNodes);
		}
                          ...
                          struct Section *fileSections[], struct FileStackNode fileNodes[])
{
	...
	tryReadstr(assert->message, file, "%s: Cannot read assertion's message: %s", fileName);
}
...
void obj_ReadFile(char const *fileName, unsigned int fileID)
{
	...
	int matchedElems = fscanf(file, RGBDS_OBJECT_VERSION_STRING, &versionNumber);
	...
	if (versionNumber != RGBDS_OBJECT_VERSION_NUMBER)
		errx(1, "\"%s\" is an incompatible version %u object file", fileName,
		     versionNumber);
	...
	uint32_t *nbSymPerSect = calloc(nbSections ? nbSections : 1, sizeof(*nbSymPerSect));
	...
	struct Section **fileSections =
	    malloc(sizeof(*fileSections) * (nbSections ? nbSections : 1));
	...
	for (uint32_t i = 0; i < nbSections; i++) {
		...
		if (nbSymPerSect[i]) {
			fileSections[i]->symbols =
			    malloc(nbSymPerSect[i] * sizeof(*fileSections[i]->symbols));
...
void obj_Setup(unsigned int nbFiles)
{
	...
	if (nbFiles > SIZE_MAX / sizeof(*nodes))
		fatal(NULL, 0, "Impossible to link more than %zu files!",
		      SIZE_MAX / sizeof(*nodes));
