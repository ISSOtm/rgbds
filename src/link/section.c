...
static void mergeSections(struct Section *target, struct Section *other, enum SectionModifier mod)
{
	if (target->type != other->type)
		errx(1, "Section \"%s\" is defined with conflicting types %s and %s", other->name,
	...
	if (other->isAddressFixed) {
		if (target->isAddressFixed) {
			if (target->org != other->org)
				errx(
				    1,
				    "Section \"%s\" is defined with conflicting addresses $%" PRIx16
				    " and $%" PRIx16,
				    other->name, target->org, other->org);
		} else if (target->isAlignFixed) {
			if ((other->org - target->alignOfs) & target->alignMask)
				errx(1,
				     "Section \"%s\" is defined with conflicting %" PRIu16
				     "-byte alignment (offset %" PRIu16 ") and address $%" PRIx16,
		...
		if (target->isAddressFixed) {
			if ((target->org - other->alignOfs) & other->alignMask)
				errx(1,
				     "Section \"%s\" is defined with conflicting address $%" PRIx16
				     " and %" PRIu16 "-byte alignment (offset %" PRIu16 ")",
				     other->name, target->org, other->alignMask + 1,
				     other->alignOfs);
		} else if (target->isAlignFixed
	...
	if (other->isBankFixed) {
		if (!target->isBankFixed) {
			...
			errx(1,
			     "Section \"%s\" is defined with conflicting banks %" PRIu32
			     " and %" PRIu32,
	...
	case SECTION_FRAGMENT:
		...
		if (sect_HasData(target->type)) {
			...
			target->data =
			    realloc(target->data, sizeof(*target->data) * target->size + 1);
			if (!target->data)
...
void sect_AddSection(struct Section *section)
{
	...
	if (other) {
		...
		errx(1, "Section \"%s\" is of type %s, which cannot be unionized", section->name,
		     typeNames[section->type]);
	} else {
		/* If not, add it */
...
#define fail(...)                          \
	...
	if (is32kMode && section->type == SECTTYPE_ROMX) {
		if (section->isBankFixed && section->bank != 1)
			fail("%s: ROMX sections must be in bank 1 with option -t.", section->name);
		else
	...
	if (isDmgMode && section->type == SECTTYPE_VRAM && section->bank == 1)
		fail("%s: VRAM bank 1 can't be used with option -d.", section->name);
	...
	if (section->isAlignFixed && (section->alignMask & startaddr[section->type]))
		fail("%s: %s sections cannot be aligned to $%" PRIx16 " bytes", section->name,
		     typeNames[section->type], section->alignMask + 1);
	...
	if (section->isBankFixed && section->bank < minbank && section->bank > maxbank)
		fail(minbank == maxbank ? "Cannot place section \"%s\" in bank %" PRIu32
		                          ", it must be %" PRIu32
		                        : "Cannot place section \"%s\" in bank %" PRIu32
		                          ", it must be between %" PRIu32 " and %" PRIu32,
		     section->name, section->bank, minbank, maxbank);

	/* Check if section has a chance to be placed */
	if (section->size > maxsize[section->type])
		fail("Section \"%s\" is bigger than the max size for that type: %#" PRIx16
		     " > %#" PRIx16,
		...
		if (section->isAddressFixed) {
			if (section->org & section->alignMask)
				fail("Section \"%s\"'s fixed address doesn't match its alignment",
				     section->name);
			section->isAlignFixed = false;
		} else if ((endaddr(type) & section->alignMask) == startaddr[type]) {
	...
	if (section->isAddressFixed) {
		...
		if (section->org < startaddr[section->type]
			...
			fail("Section \"%s\"'s fixed address %#" PRIx16
			     " is outside of range [%#" PRIx16 "; %#" PRIx16 "]",
			     section->name, section->org, startaddr[section->type],
			     endaddr(section->type));

		if (section->org + section->size > endaddr(section->type) + 1)
			fail("Section \"%s\"'s end address %#" PRIx16
			     " is greater than last address %#" PRIx16,
